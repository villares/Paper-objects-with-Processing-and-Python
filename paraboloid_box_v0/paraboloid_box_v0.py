...
def draw():
    ...
    # Draw 3D
    with push_matrix():
        # Comment out if using with PeasyCam
    ...
    # Draw 2D unfolded
    if export:
        begin_record(PDF, SKETCH_NAME + ".pdf")
    # cam.begin_hud()  # for use with PeasyCam
    with push_matrix():
        translate(width * 0.66, 450)
        rotate(-HALF_PI)
        if export:
            scale(10, 10)
...
def key_pressed():
    ...
    if key == "f" and dh > 5:
        dh -= 5
    if key_code == UP and box_d + box_w < 220:
        box_d += 5
    if key_code == DOWN and box_d > 5:
        box_d -= 5
    if key_code == RIGHT and box_w + box_d < 220:
        box_w += 5
    if key_code == LEFT and box_w > 5:
        box_w -= 5
